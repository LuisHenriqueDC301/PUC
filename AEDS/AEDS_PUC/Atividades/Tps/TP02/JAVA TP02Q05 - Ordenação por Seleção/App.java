...
public class App {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        // Selecao 
        int num = subarray.size();
        for (int i = 0; i < (num - 1); i++) {
            int menor = i;
            for (int j = (i + 1); j < num; j++) {
                // Comparação do primeiro caractere
                if (subarray.get(j).name.charAt(0) < subarray.get(menor).name.charAt(0)) {
                    comparacoes++;
                    menor = j;
                } else if (subarray.get(j).name.charAt(0) == subarray.get(menor).name.charAt(0)) {
                    // Se o primeiro caractere for igual, comparar o segundo caractere
                    if (subarray.get(j).name.charAt(1) < subarray.get(menor).name.charAt(1)) {
                        comparacoes++;
                        menor = j;
                    } else if (subarray.get(j).name.charAt(1) == subarray.get(menor).name.charAt(1)) {
                        // Se o segundo caractere for igual, comparar o terceiro
                        if (subarray.get(j).name.charAt(2) < subarray.get(menor).name.charAt(2)) {
                            comparacoes++;
                            menor = j;
                        }
                    }
        ...
        FileWriter logWriter = new FileWriter("863566_selecao.txt");
    ...
    public static void swap(ArrayList<Pokemon> array, int menor, int i){
        Pokemon tmp = array.get(menor);
        array.set(menor, array.get(i));
        array.set(i, tmp);
    }
}
