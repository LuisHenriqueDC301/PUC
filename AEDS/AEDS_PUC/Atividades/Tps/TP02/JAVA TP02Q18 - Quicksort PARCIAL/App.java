...
public class App {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        quicksortRec(subarray,0,subarray.size() - 1);
        ...
        FileWriter logWriter = new FileWriter("863566_QuicksortParcial.txt");
    ...
    public static void quicksortRec(ArrayList<Pokemon> array, int esq, int dir) {
        if (esq >= dir) return; // Condição de base para evitar chamadas desnecessárias

        int i = esq, j = dir;
        Pokemon pivo = array.get((esq + dir) / 2);

        while (i <= j) {
            // Primeiro while para encontrar elementos menores que o pivô
            while (i < array.size() && 
                   (array.get(i).getGeneration() < pivo.getGeneration() || 
                   (array.get(i).getGeneration() == pivo.getGeneration() && 
                    array.get(i).getName().compareTo(pivo.getName()) < 0))) {
                i++;
            }

            // Segundo while para encontrar elementos maiores que o pivô
            while (j >= 0 && 
                   (array.get(j).getGeneration() > pivo.getGeneration() || 
                   (array.get(j).getGeneration() == pivo.getGeneration() && 
                    array.get(j).getName().compareTo(pivo.getName()) > 0))) {
                j--;
            }

            // Se i <= j, fazemos a troca
            if (i <= j) {
                swap(array, i, j);
                i++;
                j--;
    ...
    public static void swap(ArrayList<Pokemon> array, int i, int j) {
        Pokemon temp = array.get(i);  // Corrige para índices baseados em 0
