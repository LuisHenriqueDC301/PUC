...
int compararDatas(Date d1, Date d2)
{
    int result = 0;
...
void bubbleSortOpt(Pokemon **array, int length)
{
    ...
    for (int i = 0; i < length - 1; i++)
    {
        ...
        {
            Pokemon *pk1 = array[j];
            Pokemon *pk2 = array[j + 1];
            if (pk1->id > pk2->id || (pk1->id > pk2->id && strcmp(pk1->name, pk2->name) > 0))
            {
...
void swap1(Pokemon** array, int i, int j) {
    Pokemon* temp = array[i - 1];  // Corrigido para ponteiro
    array[i - 1] = array[j - 1];
    array[j - 1] = temp;
    movimentao += 3; // Considerando 3 movimentações
}
...
void construir(Pokemon** array, int tamHeap) {
    for (int i = tamHeap; i > 1 && compare(array[i - 1], array[(i / 2) - 1]) > 0; i /= 2) {
        swap1(array, i, i / 2);
    }
}
...
int getMaiorFilho(Pokemon** array, int i, int tamHeap) {
    int filho;
    if (2 * i + 1 > tamHeap || compare(array[(2 * i) - 1], array[(2 * i + 1) - 1]) > 0) {
        comparacoes++;
        filho = 2 * i; // Se for o único filho ou maior
    } else {
        filho = 2 * i + 1;
    }
    return filho;
...
void reconstruir(Pokemon** array, int tamHeap) {
    ...
    while (i <= (tamHeap / 2)) {
        ...
        if (compare(array[i - 1], array[filho - 1]) < 0) {
            comparacoes++;
            swap1(array, i, filho);
            i = filho;
        } else {
            break; // Para sair do loop quando a ordem está correta
        }
    }
}
...
int main()
{
    ...
    FILE *logWriter = fopen("863566_bolha.txt", "w");
    if (logWriter == NULL)
    {
