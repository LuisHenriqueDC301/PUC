...
public class App {
    ...
    for (int i = 1; i < num; i++) {
        Pokemon tmp = subarray.get(i);
        int j = i - 1;
        while (j >= 0 && 
            (subarray.get(j).captureDate.compareTo(tmp.captureDate) > 0 ||
                (subarray.get(j).captureDate.compareTo(tmp.captureDate) == 0 &&
                subarray.get(j).name.compareTo(tmp.name) > 0))) {
            comparacoes += 3;
        ...
        FileWriter logWriter = new FileWriter("863566_insercaoParcial.txt");
