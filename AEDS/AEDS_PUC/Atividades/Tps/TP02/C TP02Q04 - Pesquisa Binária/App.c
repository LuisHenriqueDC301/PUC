...
int main()
{
   ...
   while (strcmp(entrada, "FIM") != 0)
    {
        if(pesquisaBinaria(entrada, subpks, i)){
            printf("SIM\n");
        }else{
            printf("NAO\n");
        }
    ...
    fim = clock();
    FILE *logWriter = fopen("863566_binario.txt", "w");
    if (logWriter == NULL) {
        printf("Error opening file!\n");
        return 1;
    }
    
    fprintf(logWriter, "%s\t%f\t%d\n", matricula, (double)(fim - inicio) / CLOCKS_PER_SEC, comparacoes);
    fclose(logWriter);
