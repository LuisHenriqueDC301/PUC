...
void insercaoPorCor(Pokemon **array, int n, int cor, int h){
    for (int i = (h + cor); i < n; i += h) {
        Pokemon *tmp = array[i]; // Guardar a referência ao Pokemon

        int j = i - h;
        
        // Ordenar pelo campo 'weight' (peso)
        while (j >= 0 && 
               (array[j]->weight > tmp->weight || 
               (array[j]->weight == tmp->weight && strcmp(array[j]->name, tmp->name) > 0))){
            comparacoes++;
            array[j + h] = array[j]; // Movendo o ponteiro para o Pokemon
            movimentao++;
            j -= h;
        }
        array[j + h] = tmp; // Colocar o Pokemon na posição correta
        movimentao++;
    }
}
...
void shellsort(Pokemon **array, int n) {
    ...
    // Calcular o maior valor inicial de h
    do { 
        h = (h * 3) + 1; 
    } while (h < n);
