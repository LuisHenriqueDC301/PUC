    ...
    public void ler(String linha) throws ParseException{
        ...
        for (int j = 0; j < habilidadesList.size(); j++) {
            habilidadesList.set(j, habilidadesList.get(j)
                .replace("[", "")
                .replace("\"", "")  // Use aspas duplas corretamente
                .replace("]", "")
                .replace("'", "")
                .trim());
        }
        ...
        this.setLegendary(Boolean.parseBoolean(array[c++]));
        
        //Convertendo data
        SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");  // Ajuste o formato conforme a data
