...
// Função para inserir um Pokémon no final da fila
void inserirFim(Pokemon *p, Fila *fila) {
    if (filaCheia(fila)) {
        // Remove o Pokémon do início se a fila estiver cheia
        Node *removido = fila->front;
        printf("(R) %s %d %.1f\n", removido->pokemon->name, removido->pokemon->generation, removido->pokemon->height);

        fila->front = fila->front->next;
        if (fila->front == NULL) {
            fila->rear = NULL;
        }
        free(removido->pokemon);
        free(removido);
        fila->size--;
    }

    // Insere o novo Pokémon no final
    ...
    fila->rear = novo;
    fila->rear->next = fila->front;  // Mantém a fila circular
    fila->size++;
}

// Função para remover o Pokémon do início da fila
Pokemon *removerInicio(Fila *fila) {
    ...
    if (fila->front == fila->rear) {
        ...
        fila->front = fila->front->next;
        fila->rear->next = fila->front;  // Mantém a circularidade
    }
...
// Função para mostrar os Pokémon na fila circular
void mostrarFila(Fila *fila) {
    ...
    int pos = 0;
    do {
        printf("[%d] %s %d %.1f\n", pos++, atual->pokemon->name, atual->pokemon->generation, atual->pokemon->height);
        atual = atual->next;
    } while (atual != fila->front);
}

// Função para calcular e imprimir a média arredondada de captureRate na fila
void imprimirMediaCaptureRate(Fila *fila) {
    if (filaVazia(fila)) {
        printf("Média: 0\n");
        return;
    }
    ...
    Node *atual = fila->front;
    do {
        soma += atual->pokemon->captureRate;
        atual = atual->next;
    } while (atual != fila->front);

    int mediaArredondada = round((double)soma / fila->size);
    printf("Média: %d\n", mediaArredondada);
}
Pokemon **lerarquivo()
{
...
int main()
{
    ...
    while (strcmp(entrada, "FIM") != 0) {
        ...
        inserirFim(pks[numero - 1], &fila);
        imprimirMediaCaptureRate(&fila);
        scanf("%[^\r\n]%*c", entrada);
    }
    ...
    for (int i = 0; i < n; i++) {
       ...
       if (strcmp(entrada, "I") == 0) {
            int number = 0;
            scanf("%d", &number);
            inserirFim(pks[number - 1], &fila);
            imprimirMediaCaptureRate(&fila);
        } else if (strcmp(entrada, "R") == 0) {
            Pokemon *removido = removerInicio(&fila);
            if (removido) {
                printf("(R) %s %d %.1f\n", removido->name, removido->generation, removido->height);
            }
        }
    }
