...
Celula* partition(Celula* low, Celula* high) {
    ...
    for (Celula* j = low; j != high; j = j->prox) {
        if (j->pokemon->generation < pivo->generation ||
           (j->pokemon->generation == pivo->generation &&
            strcmp(j->pokemon->name, pivo->name) < 0)) {
...
void quicksortRec(Celula* low, Celula* high) {
    if (high != NULL && low != high && low != high->prox) {
        Celula* p = partition(low, high);
        quicksortRec(low, p->ant);
        quicksortRec(p->prox, high);
    }
}

void quicksort(Celula* head) {
    Celula* tail = head;
...
// Função para inserir uma célula no final da lista
Celula* inserirFim(Celula* head, Pokemon* pokemon) {
    Celula* nova = criarCelula(pokemon);
    if (head == NULL) {
        return nova;
    }
    Celula* atual = head;
...
int main()
{
    ...
    while (strcmp(entrada, "FIM") != 0)
    {
        ...
        lista = inserirFim(lista, clone(pks[numero - 1]));
        i++;
        // Ler próxima entrada
        scanf("%[^\r\n]%*c", entrada);
    }
    ...
    Celula* atual = lista;
    while (atual != NULL) {
        imprimir(atual->pokemon);  // Chama a função 'imprimir' para cada 'Pokemon'
        atual = atual->prox;       // Avança para a próxima célula da lista
    }


    
    FILE *logWriter = fopen("863566_quicksort2.txt", "w");
    if (logWriter == NULL)
    {
