...
typedef struct Celula {
    Pokemon* elemento;  // Elemento inserido na célula
...
void inserir(Pokemon* x, int pos, Lista* lista) {
    if (pos == 0) {
        ...
        if (tmp != NULL) {
            nova->prox = tmp->prox;
            tmp->prox = nova;
            if (nova->prox == NULL) { // Atualizar o último caso seja a última posição
                lista->ultimo = nova;
            }
        }
    }
...
Pokemon* removerInicio(Lista* lista) {
    if (lista->primeiro == NULL) return NULL; // Lista vazia
    Celula* tmp = lista->primeiro;
    Pokemon* elementoRemovido = tmp->elemento;
    lista->primeiro = lista->primeiro->prox;
    if (lista->primeiro == NULL) { // Lista agora está vazia
        lista->ultimo = NULL;
    }
    free(tmp);
...
Pokemon* removerFim(Lista* lista) {
    if (lista->primeiro == NULL) return NULL; // Lista vazia
    Pokemon* elementoRemovido = lista->ultimo->elemento;
    if (lista->primeiro == lista->ultimo) { // Apenas um elemento na lista
        free(lista->ultimo);
...
Pokemon* remover(int pos, Lista* lista) {
    ...
    if (tmp == NULL || tmp->prox == NULL) return NULL; // Posição inválida

    Celula* removerCelula = tmp->prox;
    Pokemon* elementoRemovido = removerCelula->elemento;
    tmp->prox = removerCelula->prox;
    if (removerCelula == lista->ultimo) { // Atualizar o último se necessário
        lista->ultimo = tmp;
    }
    free(removerCelula);
    return elementoRemovido;
