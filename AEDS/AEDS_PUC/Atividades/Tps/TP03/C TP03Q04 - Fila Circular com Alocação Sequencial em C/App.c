...
void inserirFim(Pokemon *x, Fila *fila) {
    if (filaCheia(fila)) {
        Pokemon *removido = fila->pks[fila->front];
        printf("(R) %s %d %.1f\n", removido->name, removido->generation, removido->height);

        // Marca a posição como nula para evitar acessos futuros ao mesmo ponteiro
        fila->pks[fila->front] = NULL;

        // Libera o Pokémon removido
        free(removido);

        // Atualiza o ponteiro front e o tamanho da fila
        fila->front = (fila->front + 1) % TAM_FILA;
        fila->size--;
    }

    fila->rear = (fila->rear + 1) % TAM_FILA;
    fila->pks[fila->rear] = x;  // Insere o novo Pokémon no final da fila
    fila->size++;
}
...
void mostrarFila(Fila *fila) {
    for (int i = 0; i < fila->size; i++) {
        int pos = (fila->front + i) % TAM_FILA;
        Pokemon *pk = fila->pks[pos];
        printf("[%d] ");
        imprimir(pk);
    }
}

// Função para remover do início da fila circular
...
Pokemon *removerInicio(Fila *fila) {
    ...
    Pokemon *removido = fila->pks[fila->front];
    fila->pks[fila->front] = NULL;  // Prevenir acessos futuros
    fila->front = (fila->front + 1) % TAM_FILA;
    fila->size--;
    return removido;
}
