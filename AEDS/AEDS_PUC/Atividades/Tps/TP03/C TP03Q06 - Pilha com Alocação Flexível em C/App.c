...
Pokemon *removerFim(Pilha *pilha)
{
    ...
    Pokemon *elementoRemovido = i->elemento;
...
void inserirFim(Pokemon *x, Pilha *pilha)
{
    Celula *nova = novaCelula(x);
    nova->prox = NULL; // Inicializa o campo prox da nova célula como NULL

    if (pilha->primeiro == NULL)
    {                           // Verifica se a pilha está vazia
        pilha->primeiro = nova; // A nova célula se torna a primeira
    }
    else
    {
        Celula *i = pilha->primeiro;
        while (i->prox != NULL)
        { // Percorre até o fim da pilha
            i = i->prox;
        }
        i->prox = nova; // Insere a nova célula no final
    }
}

Pokemon **lerarquivo(char *url)
{
...
int main()
{
    Pokemon **pks = lerarquivo("/tmp/pokemon.csv");
    // Lista
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (strcmp(entrada, "I") == 0)
        {
            int number = 0;
            scanf("%d", &number);
            inserirFim(pks[number - 1], pilha);
        }
        else if (strcmp(entrada, "R") == 0)
        {
            printf("(R) %s\n", removerFim(pilha)->name);
        }
    }
